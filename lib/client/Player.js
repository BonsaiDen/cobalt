...
util.inherit(Player, Emitter, {
    ...
    send: function(event) {
        if (this === this._client.getPlayer()) {
            this._client.__addPlayerEvent(event);
        }
    },

    setOwner: function() {
        return this._client.__setRoomOwner(this);
    },
    ...
    // Internal ---------------------------------------------------------------
    __setOwner: function(isOwner) {
        this._isOwner = isOwner;
        this.log('Is now owner of room');
        this.emit('owner', isOwner);
    },

    __setRandomIndex: function(index) {
        this._randomIndex = index;
    },

    __resetReceivedEvents: function() {
        this._receivedEvents.length = 0;
    },

    __addReceivedEvent: function(event) {
