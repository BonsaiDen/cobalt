...
Room.prototype = {
    ...
    _load: function() {
        ...
        // Notify the players to load the room and send randomized setup data
        this._send(
            Action.SERVER_MESSAGE_ROOM_LOAD, [

                // Random seed for PRNG
                Math.round(Math.random() * 65536),

                // Randomized player ID array
                this._players.map(function(player) {
                    return player.getId();
    ...
    _update: function() {
        ...
        // Check if we just loaded the room and tell clients
            ...
            for(i = 0; i < l; i++) {
                ...
                this._players[i].send(Action.SERVER_MESSAGE_ROOM_STARTED);
            }
