...
// Statics --------------------------------------------------------------------
Room.MAX_EVENTS_PER_PLAYER_PER_TICK = 10; // TODO use for limiting
...
Room.prototype = {
    ...
    start: function(secondsLeft) {
        ...
        if (secondsLeft > 0) {
            ...
            setTimeout(this._countdown.bind(this, secondsLeft));
    ...
    addPlayerEvents: function(player, events) {
        ...
        var maxEvents = this._server.getConfig().maxPlayerEvents;
    ...
    // Internal ---------------------------------------------------------------
    _countdown: function(secondsLeft) {

        this.log(secondsLeft.toString());
        this._send(Action.SERVER_MESSAGE_ROOM_COUNTDOWN, secondsLeft);
        ...
        if (secondsLeft === 0) {
            ...
            this._countdownTimeout = setTimeout(
                this._countdown.bind(this, secondsLeft - 1),
                1000
            );
        }
    ...
    _loop: function() {
        ...
        // Otherwise we do high precision timing via setImmediate
            ...
            // TODO if we destroy the room, this might cause issues if timed badly?
            setImmediate(this._boundLoop);
        }
        ...
        if (loaded && this._isLoaded && latest === this._tickCount) {
            ...
            for(i = 0; i < l; i++) {
                this._players[i].send(
                    ...
                    [this._tickCount % 256, this._events]
                );
            }
