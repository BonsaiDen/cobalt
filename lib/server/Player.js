...
Player.prototype = {
    ...
    _roomJoin: function(data) {
        ...
        if (!util.isValidRoomId(data[0])) {
            ...
            var room = this._server.getRoomById(data[0]); // TODO validate ID
            if (room) {
    ...
    _roomCreate: function(data) {
        ...
        var minPlayers = this._server.getMinPlayerCount(),
            maxPlayers = this._server.getMaxPlayerCount();

        if (!util.isValidRoomName(data[0])) {
            this._sendError(Action.ERROR_INVALID_PARAMETER);

        } else if (!util.isValidPlayerCount(data[1], minPlayers, maxPlayers)) {
            this.log('Invalid min player count for room');
            this._sendError(Action.ERROR_INVALID_PARAMETER);

        } else if (!util.isValidPlayerCount(data[2], minPlayers, maxPlayers)) {
            this.log('Invalid max player count for room');
            this._sendError(Action.ERROR_INVALID_PARAMETER);

        } else if (!util.isValidTicksPerSecond(data[3])) {
            ...
            this.joinRoom(this._server.createRoom(
                this._gameIdent,
                this._gameVersion,
                data
            ));
    ...
    _roomStart: function(data) {
        ...
        } else if (!util.isValidCountdown(data)) {
            this._sendError(Action.ERROR_INVALID_PARAMETER);
    ...
    _roomConfirmTick: function(data) {
        ...
        if (this._room.isStarted() && this._room.isLoaded()) {
            ...
            if (util.isValidTickCount(data[0]) && data[0] > this._tickCount) {

                if (util.isValidEventData(data[1])) {
