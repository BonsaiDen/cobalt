...
// Lockstep Server ------------------------------------------------------------
function Server(config, networkInterface) {
    ...
    this._socket = new (networkInterface || lithium).Server(
...
Server.prototype = {
    ...
    _onRemoteMessage: function(remote, msg) {
        ...
        if (!util.isValidMessage(msg)) {
            ...
            // Create a new player
                ...
                remote.accept();
                remote.player = this._createPlayer(remote, data, name);
                remote.send([
                    msg[0],
    ...
    _createPlayer: function(remote, data, name) {

        // Generate a player ID, we don't want collisions so we need to try
        // again if we find one. In theory this could loop forever but we assume
        // that in practice this is extremely unlikely
        // (unless Math.random() just keeps returning the same values)
        var id = Math.floor(Math.random() * 65536);
        while(this._playerMap.hasOwnProperty(id)) {
            id = Math.floor(Math.random() * 65536);
        }

        // Create the player and add it to all internal list / maps
        var player = new Player(this, remote, id, data[1], data[2], name);
